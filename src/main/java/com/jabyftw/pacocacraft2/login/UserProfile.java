package com.jabyftw.pacocacraft2.login;
...
import com.jabyftw.pacocacraft2.login.commands.ChangeUsernameCommand;
import com.jabyftw.pacocacraft2.profile_util.*;
...
public class UserProfile extends BasePlayerProfile {
     ...
     * NOTE: player name can be changed in game
     *
    ...
    public boolean attemptLogin(@NotNull String encryptedPassword) {
        ...
        try {
            ...
            BukkitScheduler.runTask(() -> {
                ...
                // Restore chat state set at player set up
                ChatProfile chatProfile = getPlayerHandler().getProfile(ChatProfile.class);
                chatProfile.setChatState(ChatState.EVERY_MESSAGE);
                chatProfile.setUsingFixedChat(false);
            });
            return true;
        } catch (Exception e) {
            e.printStackTrace();
     ...
     * @see LoginListener#onChat(AsyncPlayerChatEvent) should be a volatile now
     */
     ...
     * @see ChangeUsernameCommand#onChangeUsername(PlayerHandler, String, String) so this is ran asynchronously
     */
